...
static void vec2ind_mod(const float x[6000], double y[2000]);
...
static void vec2ind_mod(const float x[6000], double y[2000])
{
  ...
  for (j = 0; j < 2000; j++) {
    ...
    if (ixstart < j + 4001) {
      for (ix = ixstart + 1999; ix + 1 <= j + 4001; ix += 2000) {
        cindx++;
        if (x[ix] > mtmp) {
          mtmp = x[ix];
          itmp = cindx;
...
// Arguments    : const float max_prob[6000]
//                float max_prob_smoothed[10000]
// Return Type  : void
//
void smooth_probs_3c(const float max_prob[6000], float max_prob_smoothed[10000])
{
  double as_ind[2000];
  int sequence_start;
  ...
  for (i = 0; i < 2000; i++) {
    if ((as_ind[i] != 1.0) && (1 + i < 2000)) {
      ...
      if ((sequence_length < 40.0) && (sequence_start != 0)) {
        //  half a second:
...
// Return Type  : void
//
void smooth_probs_3c_initialize()
{
  rt_InitInfAndNaN(8U);
}
...
// Return Type  : void
//
void smooth_probs_3c_terminate()
{
  // (no terminate code required)
}

//
// File trailer for smooth_probs_3c.cpp
//
// [EOF]
//
